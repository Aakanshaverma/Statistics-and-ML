...
#question 1
# a)	Show only even numbered product ids with odd numbered price.

SELECT 	PRODUCT_ID, PRODUCT_PRICE 
FROM PRODUCT
WHERE PRODUCT_ID%2=0 AND PRODUCT_PRICE%2 !=0;

#b.	Show details of the customers whose first and last names end with vowel.

SELECT * FROM ONLINE_CUSTOMER
WHERE RIGHT(CUSTOMER_FNAME,1) IN ("A","E","I","O","U")
AND RIGHT(CUSTOMER_LNAME,1) IN ("A","E","I","O","U");


#c.	Total how many quantities of least expensive product has been sold?


SELECT P.PRODUCT_DESC, MIN(P.PRODUCT_PRICE) AS LEAST_EXPENSIVE,SUM(OI.PRODUCT_QUANTITY) AS TOTAL_QTY
...
#d.	Following query lists products which are not ordered. Write an equivalent query to it without using subquery:
SELECT P.product_id, P.PRODUCT_DESC 
FROM Product P LEFT JOIN  ORDER_ITEMS OI
ON OI.PRODUCT_ID=P.PRODUCT_ID
WHERE OI.PRODUCT_ID IS NULL ;



#2.		For customers who live in the same locality (address line 2 & city) outside India, show their order details: 
#customer id, full name, no. of orders placed, and total value of orders. Show 0 for last two fields if they have not placed any order.
...
SELECT  OC.CUSTOMER_ID, CONCAT(CUSTOMER_FNAME, CUSTOMER_LNAME) AS FULL_NAME, 
		IFNULL(SUM(OI.PRODUCT_QUANTITY),0) AS TOTAL_QTY, IFNULL(SUM(OI.PRODUCT_QUANTITY * P.PRODUCT_PRICE),0) AS TOATL_VAL,
        CONCAT(ADDRESS_LINE2,' ',CITY) AS LOCAL_AREA
FROM ONLINE_CUSTOMER OC LEFT JOIN ORDER_HEADER OH 
	ON OC.CUSTOMER_ID = OH.CUSTOMER_ID AND OH.ORDER_STATUS = 'Shipped'
LEFT JOIN ADDRESS A1	 
	ON OC.ADDRESS_ID = A1.ADDRESS_ID
LEFT JOIN ORDER_ITEMS OI 
	ON OH.ORDER_ID = OI.ORDER_ID
LEFT JOIN PRODUCT P 
	ON OI.PRODUCT_ID = P.PRODUCT_ID
WHERE COUNTRY!='India' AND CONCAT(ADDRESS_LINE2,' ',CITY) IN
	(
		SELECT CONCAT(ADDRESS_LINE2,' ',CITY) 
        FROM ADDRESS A2 JOIN ONLINE_CUSTOMER OC2
			ON OC2.ADDRESS_ID = A2.ADDRESS_ID
...
SELECT ADDRESS_LINE2 FROM ADDRESS

SELECT distinct(ADDRESS_LINE2) FROM ADDRESS


#3.	Create a view to show the details of the customers outside India who have not placed any order during last year but placed during its previous year. 
#(Note: Do not use any hard-coded values for years like 2016, 2017 in the conditions)
#(Columns in the view: Customer id, Full name, Phone number, Email, City, Country, Year and Total no. of orders placed).	
...
CREATE VIEW CUST_OUTSIDE_IND AS
SELECT OC.CUSTOMER_ID, CONCAT(OC.CUSTOMER_FNAME," ", OC.CUSTOMER_LNAME) AS FULL_NAME,
OC.CUSTOMER_PHONE, OC.CUSTOMER_EMAIL, A.CITY, A.COUNTRY, 
COUNT(OH.ORDER_ID) AS TOTAL_ORDERS, OH.ORDER_STATUS, OH.ORDER_DATE
...
HAVING MAX(YEAR(OH.ORDER_DATE))=(YEAR(CURRENT_DATE())-2)
